...
class LinearRegression:
    def __init__(self, n_Epochs=10000, learning_rate=0.001, k = 1, error_threshold=1e-6):
    ...
    def fit(self, x_train, y_train, create_gif=False):
        ...
        for i in range(self.n_Epochs):
            ...
            if (i>2):
                length = len(self.mse_list)
                if (abs(self.mse_list[length-1] - self.mse_list[length-2])<self.error_threshold):
